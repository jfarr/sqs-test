...
import com.example.demo.TestProtos.Message;
...
public class SqsTestApplication {
	...
	public void receive(String message) throws IOException, ClassNotFoundException {
		List<S3EventNotificationRecord> records = readValue(
				readValue(message, SnsNotification.class).getMessage(), 
				S3EventNotification.class)
			.getRecords();
		for (S3EventNotificationRecord record : records) {
			String key = record.getS3().getObject().getKey();
			S3Object object = s3.getObject(new GetObjectRequest("f4908ccb-df04-42aa-b8a8-185daf9fdff1", key));
			System.out.println(object);
//			printProto(object.getObjectContent());
	...
	private void printProto(InputStream input) throws IOException {
		Message message = TestProtos.Message.parseDelimitedFrom(input);
		while (message != null) {
			System.out.println(message.getText());
			message = TestProtos.Message.parseDelimitedFrom(input);
		}
    }
